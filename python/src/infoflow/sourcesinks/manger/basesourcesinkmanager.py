...
from ..definitions.methodsourcesinkdefinition import MethodSourceSinkDefinition
from ..definitions.fieldsourcesinkdefinition import FieldSourceSinkDefinition
from ..definitions.stataementsourcesinkdefinition import StatementSourceSinkDefinition
...
class BaseSourceSinkManager:
    ...
    def __init__(self, sources, sinks, callback_methods=None, config=None):
        ...
        for cb in callback_methods:
            self.callback_methods[cb.target_method] = cb
    ...
    def get_signature(self, am):
        ...
        if isinstance(am, MethodSourceSinkDefinition):
            ...
            raise RuntimeError("Invalid type of source/sink definition: %s" % am.__class__.__name__)
    ...
    def get_sink_definition(self, s_call_site, manager: InfoflowManager, ap: AccessPath):
        ...
        if s_call_site.containsInvokeExpr:
            ...
            if not SystemClassHandler().is_taint_visible(ap, callee):
                return None
    ...
    @staticmethod
    def find_definition_in_hierarchy(callee: SootMethod, mehtod_map: dict):
        sub_sig = callee.name
    ...
    @staticmethod
    def create_source_info(s_call_site, manager: InfoflowManager, define):
        if define is None:
            return None
    ...
    def check_callback_param_source(self, s_call_site, cfg: InfoflowCFG):
        ...
        if isinstance(source_sink_def, MethodSourceSinkDefinition):
            ...
            if self.source_sink_config.getCallbackSourceMode() == CallbackSourceMode.SourceListOnly \
                ...
                method_param_defs = method_def.parameters
                if method_param_defs is not None and len(method_param_defs) > param_ref.index:
                    ap_tuples = method_def.parameters[param_ref.index]
                    if ap_tuples is not None and not ap_tuples.is_empty():
                        for curTuple in ap_tuples:
                            if curTuple.sink_source.is_source():
    ...
    @staticmethod
    def get_ui_source_definition(s_call_site, cfg: InfoflowCFG):
        return None
        ...
        if self.source_defs is not None:
            ...
            for entry in self.source_defs:
                ...
                if isinstance(source_sink_def, MethodSourceSinkDefinition):
                    ...
                    if return_type is None or return_type.is_empty():
        ...
        if self.sink_defs is not None:
            ...
            for entry in self.sink_defs:
                ...
                if isinstance(source_sink_def, MethodSourceSinkDefinition):
                    ...
                    if method_source_sink_def.call_type == CallType.Return:
                        ...
                        method = method_source_sink_def.method
                        return_type = method.ret
                        is_method_without_return_type = return_type is None or return_type.is_empty()
