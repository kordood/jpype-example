...
class SolverCallFlowFunction(FlowFunction):
    ...
    def compute_targets(self, d1, source):
        ...
        if res is not None and not res.isEmpty() and d1 is not None:
            for abs in res:
                self.flowfunctions.aliasing.getAliasingStrategy().injectCallingContext( abs, self.flowfunctions.solver,
                                                                                        self.flowfunctions.dest,
                                                                                        self.flowfunctions.src, source,
                                                                          d1 )
        return self.flowfunctions.notifyOutFlowHandlers( self.flowfunctions.stmt, d1, source, res, FlowFunctionType.CallFlowFunction )
